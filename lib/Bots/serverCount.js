...
function updateServerCount(client, auth, anydata) {
    return new Promise((resolve, reject) => {
        ...
        const URL = `${baseURL}/api/v1/bots/${client.user.id}/edit/servercount`
        //fetch(URL, Options)
        new utilities.request({url: `${anydata.baseURL || utilities.baseURL}/api/v1/bots/${client.user.id}/edit/servercount`, body: finalData, headers: headerss}).post()
            .then(res => res.json())
            .then(jsonData => {
